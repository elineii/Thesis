...
model {
  ...
  for (i in 1:n){
    ...
    if (y[i] != (-1)) {
      ...
      conditional_mu_transformed[, 1] = w * (gamma_2-gamma_1) - (conditional_mu[1, 1] - conditional_mu[2, 1]);
      conditional_mu_transformed[, 2] = w * (gamma_3-gamma_1) - (conditional_mu[1, 1] - conditional_mu[3, 1]);
